// import { Container } from "react-bootstrap";
import { Link } from "react-router-dom";
import ReactPlayer from "react-player/youtube";
import { PageTitle } from "@/components/Typography";

// const binder_image_url = "https://img.shields.io/badge/Image-Classification-579ACA.svg?logo=data:image/png;base64,";

// route: /about
export default function About() {
	return (
		<>
			<PageTitle>About Scivision</PageTitle>
			<div className="prose">
				<p>The Scivision project is building:</p>
				<ul className="list list-disc">
					<li>
						A <Link to="/community">community</Link> of computer vision
						practitioners in the sciences and humanities
					</li>
					<li>
						A <strong>catalog</strong> of community-curated computer vision{" "}
						<Link to="/model-grid">models</Link> and{" "}
						<Link to="/datasource-grid">datasets</Link> from the sciences and
						humanities
					</li>
					<li>
						A <strong>software ecosystem</strong> of interoperable tools and
						utilities for working with computer vision models and data including{" "}
						<a href="#scivisionpy">Scivision.Py</a> and{" "}
						<a href="https://github.com/alan-turing-institute/pixelflow">
							Pixelflow
						</a>
					</li>
				</ul>
				<div className="relative aspect-video">
					<ReactPlayer
						url="https://www.youtube.com/watch?v=B7fOBpXnO4g"
						width="100%"
						height="100%"
						className="absolute top-0 left-0"
					/>
				</div>
				<p>
					The Scivision Project is funded by{" "}
					<a href="https://www.turing.ac.uk/">The Alan Turing Institute</a>.
				</p>
			</div>
		</>
	);
}
